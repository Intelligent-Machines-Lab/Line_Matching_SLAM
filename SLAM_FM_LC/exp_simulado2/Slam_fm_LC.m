...
% CLNJ e Luciana Lemos, 16/11/2022, 04:21 h
...
Scan_LC=[21:32]; % Nos. dos scans selecionados para execução do procedimento de LC
...
for iScan=1:NScans
    ...
    if iScan == 1
       ...
       Ambiente(1).Scan_P1=Ambiente(1).Scan;
       Mapa_Global.Segmentos=Ambiente(1).Scan;
    else
       ...
       figure(iScan-1);
       [h1_SC_local,h2_SC_local]=plot_2_Ambientes(Ambiente_SC1,Ambiente_SC31);
       plot_Pose([0;0;0]);
    ...
    if iScan > 1
       ...
       if LC > 0
           ind_LC=min(abs(iScan-Scan_LC));
           if ind_LC==0 % executar o procedimento de LC
              if isempty(Marcos_MG); error('Marcos_MG está vazio!'); end
              Ambiente=loop_closure(Ambiente,Mapa_Global,Marcos_MG,iScan,Nf_LC);
              Nf_LC=Nf_LC+2;
           end
       end
       ...
       % Detectar e indicar no Mapa Global os cantos 
       Mapa_Global.Cantos=Detect_Corners(Mapa_Global.Segmentos);
       if iScan <= 13
          ind=find((7<Mapa_Global.Cantos(1,:)) & (Mapa_Global.Cantos(1,:)<9));
          Marcos_MG=Mapa_Global.Cantos(:,ind);
          %ind=find((Mapa_Global.Cantos(1,:)<5) & (Mapa_Global.Cantos(2,:)>-2));
          %Marcos_MG2=Mapa_Global.Cantos(:,ind);
       end
       %Marcos_MG=[Marcos_MG1 Marcos_MG2];
       pause(0.3);
    end
    
    % Plotar Mapa Global e Pose atual no Mapa Global
    if exist('h1_MG'); delete(h1_MG); end
...
%     end
    ...
    title(['Mapa Global após Scan ' num2str(iScan) ', No. Seg/Cantos = ' num2str(N_seg_MG) '/' num2str(N_cantos_MG)]);
...
for iScan=1:NScans
    if iScan > 1
       % Plotar linha de entre a pose atual e a pose anterior
       Pose_linha=[Ambiente(iScan-1).Pose_P1 Ambiente(iScan).Pose_P1]; hold on;
       plot(Pose_linha(1,:),Pose_linha(2,:),'k-','LineWidth',1);
...
figure(94);
yyaxis left;
plot(Erro_Poses(:,1),'b-o','LineWidth',1);hold on;plot(Erro_Poses(:,2),'b--*','LineWidth',1);hold off;
% plot(RT_fm(:,2),'bo','LineWidth',2);hold on;plot(RT_fm(:,3),'b*','LineWidth',2);hold off;
xlabel('No. Scan');
ylabel('Posição (m)');
yyaxis right;
plot(Erro_Poses(:,3)*180/pi,'r-o','LineWidth',1);hold off;
% plot(RT_fm(:,1),'ro','LineWidth',2);hold off;
ylabel('Orientação (graus)');
grid on;
legend('Erro_X','Erro_Y','Erro_\theta');
title('Comparação entre o erro da postura real e estimada');
%
figure(95);
seg=[NaN;NaN];
mapa_global = [];
for i=1:size(Mapa_Global.Segmentos,2)/2
    mapa_global = [mapa_global,[Mapa_Global.Segmentos(:,2*i-1) Mapa_Global.Segmentos(:,2*i) seg]];
end
plot(mapa_global(1,:),mapa_global(2,:),'k','LineWidth',2); hold on;
% plot(mapa_global(1,:),mapa_global(2,:),'*k','LineWidth',2);hold on;
mapa_real;
hold off;
axis equal;
...
% Scan_LC = nos. dos scans escolhidsos para execução do procedimento de LC
function Ambiente=loop_closure(Ambiente,Mapa_Global,Marcos_MG,iScan,Nf)
figure(Nf);
    ...
    if ~isempty(ind_aval)
        ...
        for i=1:size(Marcos_MG_SC3,2)
           ...
           if numel(inds)>1
               for k= 1:size(inds,2)
                   if inds(k)~=ind_aval
                       ind_dist_min(i)=inds(k);
                       dist_min(i)= dist(ind_dist_min(i));
                   end
               end 
           else
               ind_dist_min(i)=inds;
...
%------------------------------------------------------------------------------
if isempty(Seg_marcos)
    error('LC foi tentado mas não foram encontrados cantos perto dos marcos!')
end
str2_title=sprintf('Scan %d, SC local, Marcos = ',iScan);
...
Ambiente(iScan).ind_Marcos=ind_Marcos;
Ambiente(iScan).Marcos=Marcos_iScan;
...
xmax = max(Mapa_Global.Segmentos(1,:));
xmax = ceil(xmax);
xmin = min(Mapa_Global.Segmentos(1,:));
xmin = floor(xmin);
ymax = max(Mapa_Global.Segmentos(2,:));
ymax = ceil(ymax);
ymin = min(Mapa_Global.Segmentos(2,:));
ymin = floor(ymin);
...
%
Pose0=[mean(Marcos_MG(1,:)); mean(Marcos_MG(2,:)); 0];
[Pose1,feval]=Loop_Closure_Pose(Pose0,Marcos_MG,Marcos_Ang,Marcos_Dist);
...
seg3 = [0 0; 0 3];
seg4 = [8.41 8.41;0 1];
seg5 = [0 1.2;1.45 1.45];
seg6 = [2.4 2.4;3 1.8];
seg7 = [(4.52-0.3*sqrt(2)) (4.52+0.3*sqrt(2));((1.39-0.3*sqrt(2))) (1.39+0.3*sqrt(2))];
seg8 = [8.41 8.41;2 3];
seg9 = [14.6 14.6;0 3];
seg10 = [(12.285-0.3*sqrt(2)) (12.285+0.3*sqrt(2));(1.39+0.3*sqrt(2)) (1.39-0.3*sqrt(2))];
seg = [NaN;NaN];
hold on
% real_segments_global = [0 14.6 NaN 0 14.6 NaN 0 0 NaN 14.6 14.6 NaN 0 1.2 NaN 2.4 2.4 NaN 8.41 8.41 NaN 8.41 8.41 NaN (4.52-0.3*sqrt(2)) (4.52+0.3*sqrt(2)) NaN (12.285-0.3*sqrt(2)) (12.285+0.3*sqrt(2));
%                         0 0 NaN 3 3 NaN 0 3 NaN 0 3 NaN 1.45 1.45 NaN 3 1.8 NaN 0 1 NaN 2 3 NaN ((1.39-0.3*sqrt(2))) (1.39+0.3*sqrt(2)) NaN (1.39+0.3*sqrt(2)) (1.39-0.3*sqrt(2))];
real_segments_global = [seg1 seg seg2 seg seg3 seg seg4 seg seg5 seg seg6 seg seg7 seg seg8 seg seg9 seg seg10];
real_segments_local = real_segments_global - [0.5;2.5];
% plot(real_segments_global(1,:),real_segments_global(2,:));
plot(real_segments_local(1,:),real_segments_local(2,:),'r--','LineWidth',3); hold on;
plot(real_segments_local(1,:),real_segments_local(2,:),'rd','LineWidth',3);
hold on
% axis([-2 16 -2 5]);
% real_segments_local = real_segments_global - [-0.5;-2.5];
end
