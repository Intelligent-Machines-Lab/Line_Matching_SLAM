...
save segments_sc new_line_segment;
...
function corner_detection(line_segment,coeff_seg)
        ...
        for j = 1:size(segment_comparation,1)
            if comparation(j,1)==1            
                if segment_comparation(j,3)==1
                    distance(i,aux2) = segment_comparation(j,5); 
%                     aux2 = aux2+1;
                else
                    distance(i+1,aux2) = segment_comparation(j,5); 
                    aux2 = aux2+1;
                end
        ...
        distance_min_seg_endpoint1 = min(distance(i,:));
        distance_min_seg_endpoint2 = min(distance(i+1,:));
...
%     y1 = (coeffs(1,3)-coeffs(1,1)*(-2))/coeffs(1,2)
%     y2 = (coeffs(1,3)-coeffs(1,1)*(2))/coeffs(1,2)
%     y3 = (coeffs(2,3)-coeffs(2,1)*(-2))/coeffs(2,2)
%     y4 = (coeffs(2,3)-coeffs(2,1)*(2))/coeffs(2,2)
    
    for i = 1:size(segment_comparated,1)
        seg1 = segment_comparated(i,1);
        seg2 = segment_comparated(i,2);
        coeff_seg1 = coeffs(seg1,:);
        coeff_seg2 = coeffs(seg2,:);
        if coeff_seg1(1)
        end
    end
    ...
    for i = 1:N
        ...
        elseif abs(segments(1,2*i-1)-segments(1,2*i))<tol
            if segments(2,2*i-1)>segments(2,2*i)
                y1 = segments(1,2*i-1)+delta;
                y2 = segments(1,2*i)-delta;
...
%     new_segments1 = zeros(2,2*N);
    for i = 1:size(segment_comparated,1)
        seg1 = segment_comparated(i,1);
        seg2 = segment_comparated(i,2);
        end_point1 = segment_comparated(i,3);
        end_point2 = segment_comparated(i,4);
...
%         coeff_seg1 = coeffs(seg1,:);
%         coeff_seg2 = coeffs(seg2,:);
%         %O ponto da linha que e mais proximo do end point
%         end_point = segment_comparated(i,3);
%         if end_point==1
%             point = segments(:,2*seg1-1);
%         else
%             point = segments(:,2*seg1);
%         end
%         x = (coeff_seg2(2)*(coeff_seg2(2)*point(1,1)-coeff_seg2(1)*point(2,1))-coeff_seg2(1)*(-coeff_seg2(3)))/(coeff_seg2(1)^2+coeff_seg2(2)^2);
%         y = (coeff_seg2(1)*(-coeff_seg2(2)*point(1,1)+coeff_seg2(1)*point(2,1))-coeff_seg2(2)*(-coeff_seg2(3)))/(coeff_seg2(1)^2+coeff_seg2(2)^2);
%         new_segments(:,i)=[x;y];
%     end
%     aux3 = 1;
%     for i = 1:size(segment_comparated,1)
%         aux3 = 1;
%         if segment_comparated(i,1)==1
%             new_segments1(:,i)=new_segments(:,i);
%         else
%             if new_segments1(:,i)~=zeros(2,1)
%                 aux3=0;
%             else
%                 new_segments1(:,i)=new_segments(:,i);
%             end
...
%             seg1 = segment_comparated(i,1);
%             seg2 = segment_comparated(i,2);
%             index = find(segment_comparated(:,1)==seg2 & segment_comparated(:,2)==seg1);
%             end_point = segment_comparated(index,3);
%             if end_point==1
%                 new_segments1(:,2*seg2-1)=new_segments(:,i);
%             else
%                 new_segments1(:,2*seg2)=new_segments(:,i);
%             end
%         end
%     end
