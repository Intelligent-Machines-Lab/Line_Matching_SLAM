...
function [best_sample, best_cost, best_consensus, segment, coeff,inliers,outliers] = line_RANSAC4(points, maxIter, cost_th)
...
%             cost_arr(j) = huber_cost(d, delta_huber);
        ...
        % já encontrado
        if((nnz(consensus) >= nnz(best_consensus)) && (nnz(consensus) > 3))
            best_sample = sample;
            best_consensus = consensus;
            best_cost = cost_arr;
        end
