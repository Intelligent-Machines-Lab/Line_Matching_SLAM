...
%Ax+By = C
[A, B, C] = fit_line_least_square(new_points);
%em y=0;
y1=0;
x1 = (-B*y1+C)/A;
distance(1,i)=x1;
end
distances = [seg1;distance];
figure
plot(distances(1,:),distances(2,:),'-o','LineWidth',2);
% axis equal; 
axis([0 9 0 10]);
grid on;
xlabel('Distâncias reais (m)');ylabel('Distâncias medidas (m)');
title('Comparação entre distâncias reais e medidas do sensor');
